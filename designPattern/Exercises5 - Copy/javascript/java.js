...
var chart = (function() {
	 ...
	 */
	 privateChart1 = function() {
		var totalVal = 0;
		var colorIndex = 0;
		var x = (Math.cos(angleStart + angleEnd / 2) < 0) ? -1 : 1;
		var y = (Math.sin(angleStart + angleEnd / 2) < 0 )? -1 : 1;
		...
		for (var temp in data) {
			...
			console.log(Math.cos(angleStart + angleEnd / 2));
			var currentY = canva.height / 2 + (radiusCircle / 2) * Math.sin(angleStart + angleEnd / 2) * 1.5;
			console.log(Math.sin(angleStart + angleEnd / 2));
			ctx.fillStyle = "#fff";
			ctx.font = "10pt Arial";
